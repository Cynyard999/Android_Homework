...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    durationSelector.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        new MaterialDialog.Builder(Ch3Ex2Activity.this)
            ...
            .input(getString(R.string.duration_hint), durationSelector.getText(),
  ...
  private void resetTargetAnimation() {
    ...
    ObjectAnimator transparencyAnimator = ObjectAnimator.ofFloat(target, "alpha", 1.0f, 0.5f);
    transparencyAnimator.setRepeatCount(ValueAnimator.INFINITE);
    transparencyAnimator.setInterpolator(new LinearInterpolator());
    transparencyAnimator.setDuration(Integer.parseInt(durationSelector.getText().toString()));
    transparencyAnimator.setRepeatMode(ValueAnimator.REVERSE);

    // TODO ex2-3: 将上面创建的其他 ObjectAnimator 都添加到 AnimatorSet 中
    animatorSet = new AnimatorSet();
    animatorSet.playTogether(animator1, scaleXAnimator, scaleYAnimator, transparencyAnimator);
    animatorSet.start();
  }
}
